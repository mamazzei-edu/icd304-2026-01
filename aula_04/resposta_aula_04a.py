...
# Nessa resposta, vamos resolver o problema de otimização do cluster utilizando a função linprog do SciPy, 
# considerando os coeficientes da função objetivo, as restrições técnicas e os limites de capacidade.
# Além disso, vamos analisar os resultados para identificar gargalos e oportunidades de melhoria no cluster. 
# Por fim, vamos interpretar os preços-sombra para tomar decisões estratégicas sobre investimentos 
# em recursos adicionais.
# O código a seguir é uma implementação completa que inclui a definição do modelo, a execução do solver 
# e a análise dos resultados, tornando-se um guia passo a passo para entender o processo de otimização e 
...
if res.success:
    ...
    print("\n--- Resolução do Desafio de Negócios ---")
    # A oferta: 10 CPUs por R$ 250,00 (R$ 25,00 cada)
    valor_cpu_extra = precos_sombra[0]
    ganho_projetado = valor_cpu_extra * 10
    custo_pacote = 250.00
    
    print(f"Ganho extra projetado ao adicionar 10 CPUs: R$ {ganho_projetado:.2f}")
    print(f"Custo do pacote da AWS: R$ {custo_pacote:.2f}")
    
    if ganho_projetado > custo_pacote:
        print(">> RECOMENDAÇÃO: ACEITAR a oferta! O lucro supera o custo.")
    else:
        print(">> RECOMENDAÇÃO: RECUSAR a oferta! O pacote é muito caro para o retorno.")
