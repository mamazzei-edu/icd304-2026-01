...
b = [150, 160, 100] # teste com 200, 300 e 400 para storage
...
if res.success:
    ...
    ganho_projetado_cpu = valor_cpu_extra * 10
    ganho_projetado_storage = precos_sombra[2] * 10  # Considerando também o ganho de adicionar 10 unidades de Storage
    custo_pacote = 250.00
    
    print(f"Ganho extra projetado ao adicionar 10 CPUs: R$ {ganho_projetado_cpu:.2f}")
    print(f"Ganho extra projetado ao adicionar 10 unidades de Storage: R$ {ganho_projetado_storage:.2f}")
    print(f"Custo do pacote da AWS: R$ {custo_pacote:.2f}")
    
    if ganho_projetado_cpu + ganho_projetado_storage > custo_pacote:
