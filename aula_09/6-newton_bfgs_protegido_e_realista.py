...
def f_scipy(x_vec): return f_num(x_vec[0], x_vec[1])
def g_scipy(x_vec): return np.array(grad_num(x_vec[0], x_vec[1]))
...
def rodar_newton_manual(p0, tol=1e-6, max_iter=50):
    ...
    for i in range(max_iter):
        ...
        if np.linalg.norm(g) < tol: return p, i
        try:
            p = p - np.linalg.solve(H, g)
        except np.linalg.LinAlgError: return p, i
    return p, max_iter
...
print("-" * 65)
print(f"{'Newton (Manual)':<20} | {it_newton:<10} | {t_newton:<12.4f} | {f_scipy(res_newton):.6f}")
print(f"{'BFGS (SciPy)':<20} | {res_bfgs.nit:<10} | {t_bfgs:<12.4f} | {res_bfgs.fun:.6f}")
print("="*65)
