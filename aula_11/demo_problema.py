...
t0 = time.time()
res_local = minimize(
    f_rugged_cost, p_inicial, method="SLSQP", constraints=[con_local], tol=1e-6
)
...
res_global = differential_evolution(
    f_rugged_cost, bounds=limites, constraints=[con_global], seed=42
)
...
print("\n" + "=" * 70)
print(
    f"{'MÉTRICA':<25} | {'KKT / SLSQP (LOCAL)':<20} | {'GA / EVOLUTIVO (GLOBAL)':<20}"
)
print("-" * 70)
