...
pontos_iniciais = [[0.0, 0.0], [1.5, 1.5], [-0.5, 0.5], [3.0, -1.0]]
...
for idx, p_init in enumerate(pontos_iniciais):
    t0 = time.time()
    res = minimize(
        f_cost_global, p_init, method="SLSQP", constraints=[con_local], tol=1e-6
    )
...
res_global = differential_evolution(
    f_cost_global, bounds=limites, constraints=[con_global], seed=42
)
...
print("\n" + "=" * 80)
print(
    f"{'ESTRATÉGIA':<25} | {'CHUTE INICIAL':<15} | {'CUSTO FINAL':<12} | {'TEMPO (ms)'}"
)
print("-" * 80)
for i, r in enumerate(resultados_locais):
    print(
        f"KKT Local (Tentativa {i+1}) | {str(r['init']):<15} | {r['custo']:<12.6f} | {r['tempo']:.2f}"
    )
print(
    f"Evolutivo Global (GA)     | {'População':<15} | {res_global.fun:<12.6f} | {tempo_global:.2f}"
)
print("=" * 80)
